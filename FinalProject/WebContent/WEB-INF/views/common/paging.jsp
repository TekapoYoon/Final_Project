...
<div style="float:left; width:96%; text-align:center;"> 
	<a href="#none" title="처음페이지" onclick="goPage('0');"><img src="/FinalProject/_include/img/arrow_first.gif" alt="처음페이지" style="margin-bottom: 4px;height: 20px; width: auto;"/></a>&nbsp;		
		<%
		if (screenStartPageIndex > 1){
			%>
			<a href="#none" title="이전페이지" onclick="goPage('<%=screenStartPageIndex-1%>');"><img src="/FinalProject/_include/img/arrow_prev.gif" alt="이전페이지" style="margin-bottom: 4px;height: 20px; width: auto;"/></a>&nbsp;	
			<%
		}
		...
		for (int i=screenStartPageIndex; i<screenEndPageIndex ;i++){
			if (i==pageNumber){
		    	%>	
				<span style="font-size:13pt; color:#DE5E60; font-weight:bold;"><%=i+1%></span>&nbsp;
		    	<% 
			} else { 
		    	%>	
				<a href="#none" title="<%=i+1%>페이지" onclick="goPage(<%=i%>);" style="font-size:13pt; color:#ffffff; font-weight:normal;"><%=i+1%></a>&nbsp;
		    	<%		
		    }
		}
		...
		if (screenEndPageIndex < totalPageCount){
		    %>	
			<a href="#none" title="다음페이지" onclick="goPage(<%=screenEndPageIndex %>);"><img src="/FinalProject/_include/img/arrow_next.gif" alt="다음페이지" style="margin-bottom: 4px;height: 20px; width: auto;"/></a>&nbsp;
		    <%
		} // end if
	...
	<a href="#none" title="마지막페이지" onclick="goPage(<%=end_page %>);" ><img src="/FinalProject/_include/img/arrow_end.gif" alt="마지막페이지" style="margin-bottom: 4px;height: 20px; width: auto;"/></a>
</div>		
